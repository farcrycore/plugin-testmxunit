...
<cfcomponent name="varscoper" 	hint="I am a component used to find improperly scoped variables within a cfc">
	...
	<cffunction name="init" access="public" output="false" hint="I initialize an instance of the var scoper component which is used to find unscoped vars" >
		<cfargument name="fileParseText" required="true" type="string" hint="I am the string representing the code within a cfc that needs to be parsed" />
		<cfargument name="showDuplicates" required="false" type="boolean" hint="I specify if duplicate instances of variables should be shown, useful because first instance may be in comments" />
		<cfargument name="showLineNumbers" required="false" type="boolean" hint="I specify if line numbers should be logged, this adds on extra processing time" />
		<cfargument name="parseCfscript" required="false" type="boolean" hint="I specify if CFScript should be parsed">
		<cfargument name="customTags" required="false" type="array" hint="Array of custom tags to add to checklist">
		...
		<cfif isDefined("arguments.customTags")>
			<cfloop from="1" to="#arraylen(arguments.customTags)#" index="i">
				<cfset arrayappend(variables.tagTypes,arguments.customTags[i]) />
			</cfloop>
		</cfif>
	...
	<cffunction name="runVarscoper" access="public" output="false" returntype="void" hint="I run the file parsing process and populate structs that represent scoped variables within the cfc being parsed">
	...
	<cffunction name="findVars" output="false" returntype="struct" hint="I process a string (generally a block within a function) and return a struct of variables set with var statements">
		<cfargument name="stringToProcess" required="false" type="string">
		
		<cfset var RegExCFsetVar				= '<cfset+[\s]+var+([\s])+[a-zA-Z0-9_\[\"\''\]\##\.\s]+\=(.*?)\>' />
		<cfset var RegExCFArgumentVar			= "<cfargument[^>]+name=[""']([^""']+)" />
		...
		<!--- Loop over Arguments to find all variables that have been correctly var-ed within this cffunction --->
		<cfset varREFind = ReFindNoCase(RegExCFArgumentVar,arguments.stringToProcess,1,true)>
		<cfloop condition="varREFind.POS[1] NEQ 0">
			<!--- Pull out the variable name --->
			<cfset varVariableName = mid(arguments.stringToProcess,varREFind.pos[2],varREFind.len[2]) />
			...
			<cfset varREFind = ReFindNoCase(RegExCFArgumentVar,arguments.stringToProcess,currentPositionVarFind,true)>
		</cfloop>
	...
	<cffunction name="findCFsetVariables" output="false" returntype="void" hint="I scope the function and process all variables that were created with a cfset">
	...
	<cffunction name="findCFscriptVariables" output="false" returntype="void" hint="I scope the function and process all variables that were created within a cfscript block">
		<cfargument name="stringToParse" type="string" hint="I am the block of text that will be parsed to find unscoped vars">
		<cfargument name="positionToStart" type="numeric" hint="I am the starting position, this is generally right after the var-ed variables">
		<cfargument name="currentFunctionName" type="string" hint="I am the name of the current function that is being parsed" >
	...
	<cffunction name="findVarsByTag" output="false" hint="I scope for tags that can create variables, (cfquery, cffile, etc)">
	...
	<cffunction name="addToUnVarArray" access="private" output="false" returntype="void" hint="I add the found unVared variable to an array for the current function">
		...
		<cfif listFindNoCase(variables.ignoredScopes,listfirst(arguments.variableName,".")) EQ 0>
	...
	<cffunction name="countNumberOfLines" access="public" output="false" returntype="numeric" hint="I return the number of line breaks that appear in the given string">
	...
	<cffunction name="ReParserLoop" access="public" output="false" returntype="array" hint="I take a string and a regular expression, parse it and return an array of strings that match the RE">
	...
	<cffunction name="isGoodVariableName" access="public" output="false" returntype="boolean" hint="I check to see if this is a good variable name">
	...
	<cffunction name="cleanScript" access="public" output="false" returntype="string" hint="I clean out script that could be within the string that could cause variable parsing issues">
	...
	<cffunction name="getResultsStruct" access="public" output="false" returntype="struct" hint="I return the results of scraping the vars in a struct">
		
		<cfreturn variables.varscoperStruct />
	</cffunction>
	
	<cffunction name="getResultsArray" access="public" output="false" returntype="array" hint="I am an ordered array of all the results that were found">
		<cfreturn variables.OrderedUnVarArray />
	</cffunction>
	
	<cffunction name="getAllFunctionsScanned" access="public" output="false" returntype="Array" hint="Returns an array of all functions that were processed">
